...
const configuredStepGoal = Number(process.env.GOOGLE_HEALTH_STEP_GOAL || 12500);
const configuredFloorGoal = Number(process.env.GOOGLE_HEALTH_FLOOR_GOAL || 10);
const stepGoal = Number.isFinite(configuredStepGoal) && configuredStepGoal > 0 ? Math.round(configuredStepGoal) : null;
const floorGoal = Number.isFinite(configuredFloorGoal) && configuredFloorGoal > 0 ? Math.round(configuredFloorGoal) : 10;
...
async function getAccessToken() {
  ...
  if (accessTokenCache && Date.now() < accessTokenCache.expiresAt - ACCESS_TOKEN_EXPIRY_BUFFER_MS) {
...
function mergeRollupPoints(points: GoogleHealthRollupPoint[]) {
  return points.reduce<GoogleHealthRollupPoint>((merged, point) => {
    const activeMinutes = [
      ...(merged.activeMinutes?.activeMinutesRollupByActivityLevel || []),
      ...(point.activeMinutes?.activeMinutesRollupByActivityLevel || []),
    ];

    return {
      civilStartTime: merged.civilStartTime || point.civilStartTime,
      steps: {
        countSum: String(toNumber(merged.steps?.countSum) + toNumber(point.steps?.countSum)),
      },
      floors: {
        countSum: String(toNumber(merged.floors?.countSum) + toNumber(point.floors?.countSum)),
      },
      totalCalories: {
        kcalSum: toNumber(merged.totalCalories?.kcalSum) + toNumber(point.totalCalories?.kcalSum),
      },
      activeEnergyBurned: {
        kcalSum: toNumber(merged.activeEnergyBurned?.kcalSum) + toNumber(point.activeEnergyBurned?.kcalSum),
      },
      activeMinutes: {
        activeMinutesRollupByActivityLevel: activeMinutes,
      },
    };
  }, {});
}

async function fetchReconciledSteps(accessToken: string, start: DateParts, end: DateParts) {
  const url = new URL(`${GOOGLE_HEALTH_BASE_URL}/users/me/dataTypes/steps/dataPoints:reconcile`);
  url.searchParams.set(
    'filter',
    `steps.interval.civil_start_time >= "${formatDate(start)}" AND steps.interval.civil_start_time < "${formatDate(end)}"`,
  );
  ...
  if (!response.ok) {
    throw new Error('Google Health steps reconcile failed');
  }

  const points = (data.dataPoints || []) as GoogleHealthDataPoint[];
  const latestPoint = points
    .filter(point => point.steps?.interval?.endTime || point.steps?.interval?.startTime)
    .sort((a, b) => (
      b.steps?.interval?.endTime || b.steps?.interval?.startTime || ''
    ).localeCompare(a.steps?.interval?.endTime || a.steps?.interval?.startTime || ''))[0];

  return {
    steps: Math.round(points.reduce((total, point) => total + toNumber(point.steps?.count), 0)),
    lastSampleTime: latestPoint?.steps?.interval?.endTime || latestPoint?.steps?.interval?.startTime || null,
  };
}
...
async function fetchDailyRollup(
  ...
  return mergeRollupPoints((data.rollupDataPoints || []) as GoogleHealthRollupPoint[]);
}
  ...
  try {
    ...
    const [
      ...
      fetchReconciledSteps(accessToken, start, end).catch(async error => {
        ...
        const stepsData = await fetchDailyRollup(accessToken, 'steps', start, end);
        return {
    ...
    const totalCalories = toNumber(caloriesData.totalCalories?.kcalSum);
    const activeCalories = toNumber(caloriesData.activeEnergyBurned?.kcalSum);

    return NextResponse.json({
      steps: stepsSummary.steps,
      stepGoal,
      stepGoalSource: stepGoal ? 'configured' : 'none',
      stepsLastSampleTime: stepsSummary.lastSampleTime,
      floors: Math.round(toNumber(floorsData.floors?.countSum)),
      floorGoal,
      calories: Math.round(totalCalories || activeCalories),
